 ...
 * `undefined` cuando el campo no vino (para no pisar el valor existente en un
 * PATCH) y `null` cuando vino vacio (borrar el peso).
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (peso) {
      insertPayload.peso_gramos = peso.peso_gramos;
      insertPayload.peso_unidad = peso.peso_unidad;
    }
